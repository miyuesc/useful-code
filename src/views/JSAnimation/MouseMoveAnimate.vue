<script setup lang="ts">
  ...
  const animation = (e: MouseEvent) => {
    if (box.value && gallery.value) {
      const mouseX = e.clientX,
        mouseY = e.clientY

      const xDecimal = mouseX / box.value.clientWidth,
        yDecimal = mouseY / box.value.clientHeight

      const maxX = gallery.value.offsetWidth - box.value.clientWidth,
        maxY = gallery.value.offsetHeight - box.value.clientHeight

      const panX = maxX * xDecimal * -1,
        panY = maxY * yDecimal * -1

      const animation = gallery.value.animate(
        {
...
<template>
  <div ref="box" class="animation-box">
