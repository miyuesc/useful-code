...
class EventEmitter {
  ...
  static _addListener(type: string, fn: any, context?: any, once?: boolean) {
  ...
  static addListener(type: string, fn: Function, context?: any) {
    return EventEmitter._addListener(type, fn, context)
  }

  static hasListener(type: string, fn: Function) {
    return (
  ...
  static removeListener(type: string, fn: Function) {
  ...
  static removeAllListeners(type?: string) {
    if (isNullOrUndefined(EventEmitter._events)) return EventEmitter

    // if not provide type, remove all
    if (isNullOrUndefined(type)) {
      return (EventEmitter._events = Object.create(null))
    }
