...
type CommonIdentification = '*' | ',' | '-' | '/'
type SpecialIdentification = '?' | 'L' | 'W' | '#'
...
export const daysExpressionGenerator = (panelData: DayPanelData): [string, string] => {
  const {
    checkedType,
    daySpecify,
    weekdaySpecify,
