import { isRef, onBeforeUnmount, onMounted, Ref, shallowRef } from 'vue'
import * as echarts from 'echarts/core'
import type { ComposeOption, EChartsType } from 'echarts/core'
import type { GraphicComponentOption } from 'echarts/components'
import { eLinearGradient } from '@/ts-utils/echarts'
// 水球图支撑
import 'echarts-liquidfill'

export type WaterPoloOptions = {
  waveNumber?: number
  autoInit?: boolean
}
...
export default function useWaterPoloChart(
  ...
  const updateChart = (newValue: number) => {
    ...
    const data: number[] = new Array(options!.waveNumber).fill(value)
